...
function initRichSection() {
    ...
    grid.innerHTML = vipBanner + luxuryItems.map(item => `
        <div class="luxury-item ${item.vipOnly ? 'vip-only' : ''}">
            <img src="${item.image}" alt="${item.name}" onerror="this.src='https://via.placeholder.com/400x500/d4af37/000000?text=${encodeURIComponent(item.name)}'">
...
window.requestLuxuryAccess = (id, name, price, vipOnly) => {
    ...
    console.log(`Luxury access requested for: ${name} (${price})`);
};
